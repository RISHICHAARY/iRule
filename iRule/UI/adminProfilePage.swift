...
struct adminProfilePage: View {
    ...
    var userPolls: [Polls] {
        polls.filter{$0.company == currentUser[0].company}
    }
                    ...
                    VStack{
                        HStack(spacing:35){
                            ZStack{
                                ...
                                Circle()
                                    .trim(from: 0, to:
                                            CGFloat(Float(userPollsCompleted.count)/Float(userPolls.count)))
                                    .stroke(
                                        Color.green,
                                        lineWidth: 10
                                    )
                                Text("\(Int((Float(userPollsCompleted.count)/Float(userPolls.count))*100))%")
